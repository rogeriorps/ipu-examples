...
int main (int argc, char *argv[])
{
	...
	int fd_ipu, fd_fb, isize, ovsize, osize, alpsize;
	int ret, i;
	...
	task.overlay_en = 1;
	task.overlay.alpha.mode = 1; //1 - Local alpha blending
	...
	memset(ovbuf, 0x2222, ovsize);

	// Calculate the alpha buffer size 
	alpsize = task.overlay.alpha.loc_alp_paddr =
		task.overlay.width * task.overlay.height;

	// Allocate contiguous  physical memory for overlay buffer
	ret = ioctl(fd_ipu, IPU_ALLOC, &task.overlay.alpha.loc_alp_paddr);
	if (ret < 0) {
		printf("ioctl IPU_ALLOC fail\n");
		goto done;
	}
	...
	alpbuf = mmap(0, alpsize, PROT_READ | PROT_WRITE,
		MAP_SHARED, fd_ipu, task.overlay.alpha.loc_alp_paddr);
	if (!alpbuf) {
		printf("mmap fail\n");
		ret = -1;
		goto done;
	...
	for (i=0; i < 256 ; i++) {
		...
		if(i & 0x01){
			...
			/*fill loc alpha buffer with dedicated data*/
			memset(alpbuf, 0x00, alpsize);
		}

		printf("Local alpha blending = %d\n", i & 0x01);
	...
	if (task.overlay.alpha.loc_alp_paddr)
		ioctl(fd_ipu, IPU_FREE, &task.overlay.alpha.loc_alp_paddr);
	if (inbuf)
		munmap(inbuf, isize);
	if (ovbuf)
		munmap(ovbuf, isize);
